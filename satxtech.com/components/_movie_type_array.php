...
$movie_type_array = [
  ...
  function get_movie_by_type($type){
    require('./components/_dbconnect.php');
    $sql_query = "SELECT * FROM `movies_details` WHERE `movie_type` = '$type' LIMIT 15";
    $result = mysqli_query($conn,$sql_query);
    if (mysqli_num_rows($result) > 0) {
    echo '
    <div class="sub-heading-wraper">
      <h3 class="sub-heading">'.$type.'</h3>
      <a href="/movie_type.php?type='.$type.'" class="see-more href-links">see more &gt;</a>
    </div>
